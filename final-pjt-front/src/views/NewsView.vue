  ...
  <div class="newsTable">
    <div class="container newsTable-content">      
      <div class="row newsTable-row">
        <div v-for="(news, index) in newsTable" :key="`news-${index}`" class="col-12 col-md-6 newsTable-content-col">
          <a :href="news.link" target="_blank">
            <div class="newsTable-content-box" :style="`background-image: linear-gradient(rgba(0, 0, 0, 0.8), rgba(0, 0, 0, 0.7)), url('${news.imgSrc}');`">
              <div class="newsTable-content-text">
                <div class="newsTable-content-text-title">{{ news.title }}</div>
                <div class="newsTable-content-text-content">{{ news.content }}</div>
...
export default {
  ...
  methods: {
    getMoreNews() {
      ...
      setTimeout(() => {
        this.newsTable = this.$store.state.newsTable.slice(0, (this.nowNewsPage * 24))
    ...
    createLoader() {
      ...
      setTimeout(() => {
        // 로더 생성
        loaderPlace.innerHTML = `<div class="col-12 col-md-6" style="display: flex; justify-content: center;">
